...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // Pre-validation: Check if the interaction has already been deferred or replied to.
    if (!interaction.deferred && !interaction.replied) {
      await interaction.deferReply({ ephemeral: true });
    }

    // Role validation: Check if the user is already registered.
    if (!member.roles.cache.has(ROLE_ID)) {
      const embed = createAlertEmbed("Ya estás registrado en Be+");

      // Edit the response only if it has been deferred.
    ...
    // Final response: Send the welcome embed to the user.
    return await interaction.editReply({ embeds: [embed] });
  }
};
